...
@Component({
  selector: 'app-step-one-end',
  templateUrl: './step-one-end.page.html',
  styleUrls: ['./step-one-end.page.scss'],
})
export class StepOneEndPage implements OnInit {

  titulo = "NUEVA ENTREGA DIRECTA 1 - Solicitud";
  myForm: FormGroup;
  ...
  adelante()
  {
    localStorage.setItem('tipo_operativa','END');
    if (!this.myForm.value.request_n) {
      ...
      this.loadingCtrl.create().then(l=>{
        ...
        this.consultaService.validacionSolicitud(solicitud).subscribe((res:any) => {
          ...
          this.nav.navigateForward('/nueva-entrega-directa/step-two-end');
        },err=>{
          console.log(err);
          l.dismiss();
        });
