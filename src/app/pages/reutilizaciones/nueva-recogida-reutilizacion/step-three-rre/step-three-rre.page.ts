...
@Component({
  selector: 'app-step-three-rre',
  templateUrl: './step-three-rre.page.html',
  styleUrls: ['./step-three-rre.page.scss'],
})
export class StepThreeRrePage implements OnInit {

  titulo = "NUEVA RECOGIDA DE REUTILIZACIÓN 3 - Tipo Lectura";
  myForm: FormGroup;
    ...
    this.events.subscribe('lectura',data=>{
      ...
      this.loadingCtrl.create({message:"Comprobando etiqueta..."}).then(l=>{
        ...
        this.consultaService.GetRaeeReutilizacion(data,parseInt(this.usuario.dtercero),localStorage.getItem('tipo_operativa')).subscribe((data:any)=>{
          ...
          localStorage.setItem('alt_title_rd','NUEVA RECOGIDA DE REUTILIZACIÓN 3 - RAEE: Listado RAEE');
          localStorage.setItem('alt_title_rd_5','1');
          this.nav.navigateForward('/nueva-recogida/step-three/readed');
