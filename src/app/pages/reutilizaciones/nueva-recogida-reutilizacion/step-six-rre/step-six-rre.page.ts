...
@Component({
  selector: 'app-step-six-rre',
  templateUrl: './step-six-rre.page.html',
  styleUrls: ['./step-six-rre.page.scss'],
  providers: [Geolocation]
})
export class StepSixRrePage implements OnInit {

  titulo = "NUEVA RECOGIDA DE REUTILIZACION 5 - Recogida de Reutilización Completada";
  myForm: FormGroup;
  ...
  async ngOnInit() {
    this.consultas.createLogger('Recogida de Reutilización Completada Success');
  }
  ...
  async adelante()
  {
    ...
    this.loadingCtrl.create({message:"Guardando la información de Recogida de Reutilización"}).then(l=>{
      l.present();
        ...
        if (data.informacion.pesado > 0) {
          this.alertCtrl.create({message:"¿Quiere pesar los RAEEs?", buttons: [
          {
            ...
            handler:()=>{
              ...
              this.consultas.createLogger('Datos guardados, regresando al Inicio Success');
              this.alertCtrl.create({message:"Información de Recogida de Reutilización guardada exitosamente!", buttons: ['OK']}).then(a=>a.present());
            }
          }]}).then(a=>a.present());
        }else{
          this.consultas.createLogger('Datos guardados, regresando al Inicio Success');
