...
@Component({
  selector: 'app-step-one-rre',
  templateUrl: './step-one-rre.page.html',
  styleUrls: ['./step-one-rre.page.scss'],
})
export class StepOneRrePage implements OnInit {

  titulo = "NUEVA RECOGIDA DE REUTILIZACIÓN 1 - Solicitud";
  myForm: FormGroup;
  ...
  adelante()
  {
    if (this.myForm.value.type == 'request') {
      this.consultaService.createLogger('Nueva Recogida de Reutilización con solicitud seleccionada Success');
      localStorage.setItem('tipo_operativa','RCR');
      if (!this.myForm.value.request_n) {
        ...
        this.loadingCtrl.create().then(l=>{
          ...
          this.consultaService.validacionSolicitud(solicitud).subscribe((res:any) => {
            ...
            this.nav.navigateForward('/reutilizaciones/nueva-recogida-reutilizacion/step-two-rre');
          },err=>{
            console.log(err);
            l.dismiss();
          });
      ...
      this.consultaService.createLogger('Nueva Recogida de Reutilización sin solicitud seleccionada Success');
      localStorage.setItem('tipo_operativa','RSR');
      localStorage.removeItem('solicitud');
      this.nav.navigateForward('/reutilizaciones/nueva-recogida-reutilizacion/step-two-rre');
    }
  }
