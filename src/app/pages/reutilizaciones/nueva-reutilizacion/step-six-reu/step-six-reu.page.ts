...
@Component({
  selector: 'app-step-six-reu',
  templateUrl: './step-six-reu.page.html',
  styleUrls: ['./step-six-reu.page.scss'],
        ...
        if (data.informacion.pesado > 0) {
          this.alertCtrl.create({message:"¿Quiere pesar los RAEEs?", buttons: [
          {
            ...
            handler:()=>{
              ...
              this.consultas.createLogger('Datos guardados, regresando al Inicio Success');
              this.alertCtrl.create({message:"Información de Recepción guardada exitosamente!", buttons: ['OK']}).then(a=>a.present());
            }
          }]}).then(a=>a.present());
        }else{
          this.consultas.createLogger('Datos guardados, regresando al Inicio Success');
