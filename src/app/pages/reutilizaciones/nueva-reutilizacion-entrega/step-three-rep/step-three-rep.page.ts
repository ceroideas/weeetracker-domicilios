...
@Component({
  selector: 'app-step-three-rep',
  templateUrl: './step-three-rep.page.html',
  styleUrls: ['./step-three-rep.page.scss'],
})
export class StepThreeRepPage implements OnInit {

  titulo = "NUEVA REUTILIZACIÓN ENTREGA 3 - Tipo Lectura";
  myForm: FormGroup;
    ...
    this.events.subscribe('lectura',data=>{
      ...
      this.loadingCtrl.create({message:"Comprobando etiqueta..."}).then(l=>{
        ...
        this.consultaService.GetRaee(data,parseInt(this.usuario.dtercero),localStorage.getItem('tipo_operativa')).subscribe((data:any)=>{
          ...
          localStorage.setItem('alt_title_rd','NUEVA REUTILIZACIÓN ENTREGA 3 - RAEE: Nuevo RAEE');
          localStorage.setItem('alt_title_rd_6','1');
          this.nav.navigateForward('/nueva-recogida/step-three/readed');
