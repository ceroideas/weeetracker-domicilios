...
@Component({
  selector: 'app-step-two-rep',
  templateUrl: './step-two-rep.page.html',
  styleUrls: ['./step-two-rep.page.scss'],
})
export class StepTwoRepPage implements OnInit {

  titulo = "NUEVA REUTILIZACIÓN ENTREGA 2 - Gestor Destino: Selección";
  myForm: FormGroup;
  ...
  async cargarUsuario()
  {
    ...
    if (localStorage.getItem('tipo_operativa') == 'REP') {
      // this.mostrarNuevo = false;
      let fr = JSON.parse(localStorage.getItem('fraccion_seleccionada'))

      this.consultaService.centrosEntrega(this.usuario.dtercero,fr.pidFraccion,'REP').subscribe((data:any)=>{
  ...
  adelante()
  {
    ...
    this.nav.navigateForward('/reutilizaciones/nueva-reutilizacion-entrega/step-three-rep')
  }
  ...
  seleccionar(l)
  {
    this.selected = l.pidTerceroTipoTercero;
  }

  aceptar()
  {
    let gestor = this.listadoGestores.find(x=>x.pidTerceroTipoTercero == this.selected);
  ...
  sendToSearch2(id)
  {
    ...
    this.loadingCtrl.create({message: "Obteniendo información de centro..."}).then(l=>{
      ...
      this.consultaService.direccionesEntrega(id,this.usuario.dtercero,fr.pidFraccion,'REP').subscribe((data:any)=>{
